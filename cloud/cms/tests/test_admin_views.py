...
class TestAssetFilters(TestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.asset_list_path = reverse('admin:cms_asset_changelist')
    ...
    def setUp(self):
        ...
        caches['filters'].delete(self.superuser.id)
    ...
    def check_filter_saves(self, query_string=''):
        ...
        self.assertEqual(response.redirect_chain[-1][0], f'{self.asset_list_path}?{query_string}')
    ...
    def test_customization_default(self):
        ...
        self.assertEqual(
            list(response.context_data['cl'].queryset),
    ...
    def test_customizations_all(self):
        ...
        self.assertEqual(list(response.context_data['cl'].queryset), list(Asset.objects.all().order_by('-pk')))
    ...
    def test_customizations_other(self):
        ...
        self.assertEqual(
            list(response.context_data['cl'].queryset),
            list(Asset.objects.exclude(customizations__name__in=self.superuser.customizations).order_by('-pk'))
        )
    ...
    def test_customizations_specific(self):
        ...
        self.assertEqual(
            list(response.context_data['cl'].queryset),
            list(Asset.objects.filter(customizations=customization).order_by('-pk'))
        )
