...
class TestReleaseNotes:
    ...
    def setup(self, account_factory, asset_factory, db):
        ...
        self.unrelated_user =  account_factory(email='unrelated@user.com', is_superuser=False)
    ...
    def test_get_release_note_404(self, arf):
        ...
        response = self.get_release_note(arf=arf, asset_id=non_existant_release_note_id, user=self.user)
        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert response.data == RELEASE_NOTES_NOT_FOUND

    def test_get_release_note_success(self, arf):
        response = self.get_release_note(arf=arf, asset_id=self.existing_release_note_id, user=self.user)
    ...
    def test_get_release_note_forbidden_anonymous_user_draft(self, arf):
        response = self.get_release_note(arf=arf, asset_id=self.existing_release_note_id, draft=True) # No user
    ...
    def test_get_release_note_forbidden_anonymous_user_pending(self, arf):
        response = self.get_release_note(arf=arf, asset_id=self.existing_release_note_id, pending=True) # No user
        assert response.status_code == status.HTTP_403_FORBIDDEN
        assert response.data == RELEASE_NOTES_FORBIDDEN

    def test_get_release_note_forbidden_authenticated_user_draft(self, arf):
        response = self.get_release_note(arf=arf, asset_id=self.existing_release_note_id, draft=True, user=self.non_superuser)
        assert response.status_code == status.HTTP_403_FORBIDDEN
        assert response.data == RELEASE_NOTES_DRAFT_FORBIDDEN

    def test_get_release_note_forbidden_authenticated_user_review(self, arf):
        response = self.get_release_note(arf=arf, asset_id=self.existing_release_note_id, pending=True, user=self.non_superuser)
        assert response.status_code == status.HTTP_403_FORBIDDEN
    ...
    def test_get_release_notes_success_superuser(self, arf):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data['data']) == self.release_notes_quantity * 2 # double for drafts
