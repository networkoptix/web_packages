...
def _wrap_close(cache_client, loop):
    ...
    def _wrapper(self, *args, **kwargs):
        cache_client.on_loop_close(loop)
...
class AsyncCacheClient:
    ...
    def on_loop_close(self, loop: asyncio.AbstractEventLoop):
        # If loop is being closed then pool and client instantiated with this loop
        # must be closed properly and wiped out
        if _client := self._clients.get(loop):
            loop.run_until_complete(_client.close(close_connection_pool=True))
            del self._clients[loop]
        if _pool := self._pools.get(loop):
            loop.run_until_complete(_pool.disconnect())
    ...
    def _get_connection_pool(self, write):
        ...
        index = asyncio.get_running_loop()
    ...
    def get_client(self, key=None, *, write=False) -> AsyncRedis:
        # client must be closed before loop
        loop = asyncio.get_running_loop()
        pool = self._get_connection_pool(write)
        if not (_client := self._clients.get(loop)):
            _client = self._client(connection_pool=pool, auto_close_connection_pool=False)
            self._clients[loop] = _client
