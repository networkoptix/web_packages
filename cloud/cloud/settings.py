...
4) /api/account/register
5) /api/systems/connect
6) /api/systems/disconnect
7) /api/systems/revokeToken
8) /api/systems/{cloud_system_id}/users
9) /api/systems/{cloud_system_id}/auth
10) /api/systems/{cloud_system_id}/code
11) /api/systems/{cloud_system_id}/token
12) /oauth/*
These urls need to be whitelisted because mediaserver use them.
"""
# Comment out for swagger-ui local.
CORS_URLS_REGEX = r'^(?:/oauth.*|/api/(?:account/(?:login|authKey|register)|ping|systems/(?:(?:dis)?connect|revokeToken|(?:[\w\d-]+/(?:users|auth|code|token)?))?))'
...
if TESTING or (LOCAL_ENVIRONMENT and os.environ.get('RUN_CELERY_LOCALLY')):
