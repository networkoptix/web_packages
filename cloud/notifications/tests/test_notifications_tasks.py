...
def test_send_email(mocker, db):
    mock_send = mocker.patch('notifications.engines.email_engine.send')
    user_email = f'{uuid4()}@{uuid4()}.com'
    message = baker.make(Message, user_email=user_email, message=str(uuid4()))
