...
def merge_i18ns():
    with open('src/language_i18n.json', 'r') as auto_language:
        base_file = json.load(auto_language)

    with open('src/language_i18n_static.json', 'r') as static_language:
        static_file = json.load(static_language)

    i18n_section = merge_json(base_file, static_file)

    with open("./src/language_compiled.json", "w") as outfile:
