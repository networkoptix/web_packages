...
describe('NxDevConsoleComponent', () => {
    ...
    const contexts = expectedMenu.map(({
        ...
        name, label, icon, fields: [], global: false
    }));
    const routeMock = {
    ...
    it('should load for correct section', () => {
        expect(component.sectionParam).toBe(
            expectedSection);
    });
    ...
    it('should set correct base route', () => {
        expect(component.base).toBe(
            `/developers/${expectedSection}/${expectedMode}/${expectedId}`);
    });

    it('should get correct manifest', () => {
        expect(component.manifest).toBe(
            mockManifest);
    });
