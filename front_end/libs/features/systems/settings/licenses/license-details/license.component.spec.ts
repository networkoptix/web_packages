...
const setupLicenseDetailComponent = (): ReturnType<typeof setupComponent<NxLicenseDetailComponent>> => {
    return testBedSetupFactory([
        NxContentBlockComponent,
        NxContentBlockSectionComponent,
        NxInfoBlockComponent
    ], [DatePipe])(NxLicenseDetailComponent, { licenses });
};

const getTile = (debugElement: DebugElement) => debugElement.nativeElement.querySelector('nx-block');
    ...
    describe('Have registered license key', () => {
        ...
        describe('Have elements', () => {
            ...
            it('should display license key properties', async () => {
                const { debugElement, component } = await setupLicenseDetailComponent();
                const properties = getTile(debugElement).querySelector('div.block-section-values').querySelectorAll('p');
