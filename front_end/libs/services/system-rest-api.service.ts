...
export class NxSystemRestAPI extends NxSystemAPI implements MediaserverRestConnection {
    ...
    @memoizeAsyncPersistent
    override getAPIToolManifest(): Promise<MenuManifest | LegacyMenuManifest | undefined> {
    ...
    override previewUrl(
        ...
        return this.get(endpoint, { params: data, responseType: 'blob' }).pipe(
            catchError(e => of(new Blob([401, 403].includes(e.status) ? ['unauthorized'] : []))),
    ...
    override deleteUser(userId: string): Observable<ChangedIdReturned> {
        return this.delete<ChangedIdReturned>(`/rest/v1/users/${cleanIdLegacy(userId)}`);
    }
}
