...
const getCachedResponse = <T>(requestArgs: string): Observable<T> => from(NxDbService.personal.cachedRequest.where({ requestArgs }).first(request => request.response as T).catch(() => null)).pipe(
    filter(response => !!response),
    tap(response => console.log(`${getUser()} - ${requestArgs}: ${response}`))
);

const saveResponse = <T>(requestArgs: string, response: T): Promise<string> => NxDbService.personal.cachedRequest.put({
    requestArgs,
    response,
    lastUpdate: Date.now()
});
