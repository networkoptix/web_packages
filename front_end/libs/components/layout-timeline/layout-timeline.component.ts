...
export class NxLayoutTimelineComponent {
    ...
    updateTooltip = (event: MouseEvent): void => {
        ...
        const nextRecord = this.records.find(({ start, end }) => (start <= mousePosition && end >= mousePosition) || start >= mousePosition) || { start: 0, end: 0, startTimeMs: 0, endTimeMs: 0, durationMs: 0 };
        ...
        const start = Math.max(nextRecord.start, 0, mousePosition) / this.pixelSize / 100 * this.scale - scrollLeft;
        const tooltipWidth = 216;
        const x = Math.min(Math.max(start - (tooltipWidth / canvasWrapper.clientWidth * start), 0), canvasWrapper.clientWidth - tooltipWidth);
        const selected = uniq(this.records.filter(({ start, end }) => {
            const pointer = Math.max(nextRecord.start, mousePosition);
            return start <= pointer && end >= pointer;
        }).map(({
            cameraId
        }) => this.layoutItemLookup[cameraId]?.name || cameraId));
        this.tooltipDetails = { selected, position, x, xActual: start };
    };

    updatePosition = (position?: number): void => {
        this.position = (typeof position === 'number' ? position : this.tooltipDetails.position) * 1000;
    ...
    #calcPosition = (record: Pick<TimeDetail, 'start' | 'end'>): Pick<TimeDetail, 'start' | 'end'> => {
    ...
    #drawRecord = (record: Pick<TimeDetail, 'start' | 'end'>, context: CanvasRenderingContext2D): void => {
        const { start, end } = this.#calcPosition(record);
        context.moveTo(start, 0);
        context.lineTo(end, 0);
    };

    #clear = (): void => this.timelineCanvas.nativeElement.getContext('2d').clearRect(0, 0, this.width, this.height);
