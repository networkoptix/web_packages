...
const getLeafNodes = (node, result = []) => {
    ...
    if (
        children.length &&
        children.some(child => Array.isArray(child) || child instanceof Object)
    ) {
...
const summarizeByLevel = (summary, cur) => {
...
> {
    ...
    alarmsFilteredByResource$ = this.alarms$.pipe(
        map(alarms => {
    ...
    small$ = this.alarmsFilteredByResource$.pipe(
        map(alarms => getLeafNodes(alarms).reduce(summarizeByLevel, {})),
    );
    ...
    large$ = this.alarmsFilteredByResource$.pipe(
        map(alarms => {
            const buildUpState = Object.entries(alarms).map(([key, resources]) =>
                Object.entries(resources).map(([id, items]) =>
                    Object.entries(items).map(([state, types]: any) =>
                        Object.values<any[]>(types)
                            .reduce((combined, next) => [...combined, ...next], [])
                            .map(({ level, text: message }) => {
                                const currentId = `{${id}}`;
                                const name =
                                    this.devices.find(({ id: deviceId }) => deviceId === currentId)
                                        ?.name || currentId;
                                const { resource, id: resourceId } =
                                    this.card.config.resources[key];
                                return {
                                    resource,
                                    resourceId,
                                    level,
            ...
            return getLeafNodes(buildUpState).filter(
                node => node instanceof Object && !Array.isArray(node),
            );
        }),
    );
    ...
    updateResourceHeading = (): void => {
        ...
        const otherHeading = values.reduce(
            (joined, { name }, i, arr) =>
                `${joined}${i && arr.length > 2 ? ',' : ''}${i && arr.length > 1 ? ' ' : ''}${
                    arr.length > 1 && i === arr.length - 1 ? 'and ' : ''
                }${name}`,
            '',
        ) as string;
    ...
    handleShowAction = ({
        ...
        const segments = [
            name,
            this.card.config.resources[key]?.name,
            this.selectedSystem.name,
            'Health Monitor',
        ];
        const label = segments.reduce(
            (combined, segment) =>
                !segment ? combined : combined ? `${combined} - ${segment}` : segment,
            '',
        );
    ...
    updateSystem = async (system: SystemDropdownItem, systemsToTry = []) => {
        ...
        if (!this.manifest) {
            try {
                const { reply: manifest } = await nextSystem.mediaserver
                    .getHealthManifest()
                    .toPromise();
        ...
        this.card.config.resources = (this.manifest || []).reduce(
        ...
        this.card.config.autoUpdate =
            this.card.config.autoUpdate ?? NxHealthMonitorWidgetComponent.BASE_CONFIG.autoUpdate;
        this.card.config.updateInterval =
            this.card.config.updateInterval ??
            NxHealthMonitorWidgetComponent.BASE_CONFIG.updateInterval;
        ...
        timer(0, 1000)
            ...
            .subscribe(remaining => {
                if (!remaining) {
                    ...
                    this.healthMonitorAge =
                        this.card.config.updateInterval - Math.round(remaining / 60);
                    this.updatingIn = 0;
                } else {
