...
export class NxSystemLicenseSummaryWidget extends FirstPartyWidget<typeof NxSystemLicenseSummaryWidget.BASE_CONFIG> {
    ...
    updateSystem = (systemDropdown: SystemDropdownItem): void => {
        ...
        this.card.config.refreshInterval = this.card.config.refreshInterval ?? NxSystemLicenseSummaryWidget.BASE_CONFIG.refreshInterval;
    };
    ...
    constructor(
        ...
        timer(0, 1000).pipe(
            ...
            if (!remaining) {
                ...
                this.serverSummaryAge = +this.card.config.refreshInterval - Math.round(remaining / 60);
                this.updatingIn = 0;
            } else {
