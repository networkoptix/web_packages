...
export class DynamicWeightIrregularLengthIntervalSetExpander extends AbstractIntervalSetExpander {
  ...
  public expand (
    ...
    return this.getSerifTimes(intervals[0], skipIntervals).map((when: timeStampMs) => ({
      when,
      ...this.getSerifIntervalAndWeight(when, intervals, weights),
    }))
  }

  protected getSerifIntervalAndWeight (when: timeStampMs, intervals: Array<IrregularLengthInterval>, weights) {
    for (let i = intervals.length - 1; i >= 0; i--) {
      const interval = intervals[i]
      if (isAlignedByIrregularInterval(when, interval)) {
        return {
          weight: weights ? weights[interval].get() : i + 1,
          interval,
        }
      }
    }
...
export default DynamicWeightIrregularLengthIntervalSetExpander
