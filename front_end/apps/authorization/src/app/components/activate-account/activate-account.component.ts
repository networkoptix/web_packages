...
export class NxAuthorizeActivateAccountComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        this.contentHeader$ = this.activated$.pipe(
            map((activated: boolean) => {
                return activated
                    ? this.LANG.authorize.activatedText
                    : this.LANG.authorize.createdText;
            }),
        );
        this.contentMessage$ = combineLatest([this.activated$, this.fromEmail$]).pipe(
            map(([activated, fromEmail]) => {
