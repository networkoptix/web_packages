    ...
    .factory('mediaserver', ['$http', '$uibModal', '$q', '$localStorage', '$location', '$log', 'nativeClient', 'systemAPI',
        function ($http, $uibModal, $q, $localStorage, $location, $log, nativeClient, systemAPI) {
            ...
            /* checkCurrentPassword:function(password){
                ...
                remotelogin: function(url, login, password) {
                    login = login.toLowerCase();
                    return wrapPost(proxy + '/proxy/https/' + url + '/rest/v1/login/sessions', { username: login, password }).then(({ data }) => {
                        // bearerToken = data;
                        return data;
                    });
                },
                ...
                // 1. get remote nonce
                    ...
                    return self.remotelogin(url, remoteLogin, remotePassword).then(function(res) {
                        ...
                        return wrapPost('/rest/v1/system/merge', data);
                    });
                },
