...
angular.module('webadminApp')
    ...
    function ($http, $uibModal, $q, $localStorage, $location, $log, nativeClient, systemAPI) {
        ...
        function getModuleInformation(){
            this.systemId = systemAPI.systemId;
            
            var url = window.location.protocol + '//' + Config.trafficRelayHost.replace('{systemId}', this.systemId),
                salt = (new Date().getTime()) + '_' + Math.ceil(Math.random()*1000);
            
            url += '/web/api/moduleInformation?showAddresses=true&salt=' + salt;
            url += '&auth=' + systemAPI.authGet();
        ...
        mediaserver = {
            ...
            getCurrentUser:function (forcereload){
                if(cacheCurrentUser === null || forcereload){
                    cacheCurrentUser = wrapGet(systemAPI.urlBase + '/api/getCurrentUser?auth=' + systemAPI.authGet());
                }
