...
10. Should open systems page in anonymous state
    ...
    Log In    ${system}[cloudOwner]    ${base password}    button=None  api=${False}

11. Should update owner name in systems list, if it's changed
    [Tags]    threaded
    Set Account Name    ${extra system}[cloudOwner]    ${base password}    newFirstName    newLastName
    Log In    ${system}[cloudOwner]     ${base password}
    Go To    ${ENV}/systems
    Validate on Systems Page
...
12. Search should only be visible with 9 or more systems
    [Tags]    C41890
    Disconnect from account    ${system}[cloudOwner]    ${base password}    ${extra system}[cloud id]
    Log In    ${system}[cloudOwner]    ${base password}
    Go To    ${ENV}/systems
