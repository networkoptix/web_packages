import unittest
from unittest.mock import patch, MagicMock

from tools.utils import bind_system_to_cdb_organization


class TestBindSystemToCdbOrganization(unittest.TestCase):
    @patch('tools.utils.NxCloudAPISyncClient')
    def test_user_agent_header_deleted(self, mock_client):
        # Set up the mock client and its methods
        mock_instance = MagicMock()
        mock_system = MagicMock()
        mock_bind = MagicMock()
        mock_json = MagicMock()
        mock_response = MagicMock(status_code=200, json=mock_json)

        mock_client.return_value.__enter__.return_value = mock_instance
        mock_instance.system = mock_system
        mock_system.bind = mock_bind
        mock_bind.return_value = mock_response

        # Call the function with some test data
        cloud_host = 'http://test-cloud-host'
        access_token = 'test-access-token'
        organization_id = 1234567890
        system_id = 9876543210
        name = 'Test System'
        customization = {}
        opaque = ''

        bind_system_to_cdb_organization(
            cloud_host,
            access_token,
            organization_id,
            system_id,
            name,
            customization,
            opaque)

        mock_client.assert_called_once_with(
            host=cloud_host,
            headers={'User-Agent': None})
