...
class TestServicePropertiesExpirationDate:
    ...
    def test_creating_demo_service_record(self, service_name):
        ...
        properties = ServiceToSystemProperties.objects.first()
        assert properties.cloud_system == self.system
        assert properties.service == service
        assert properties.expiration_date == record.created_ts + relativedelta(months=service.duration)