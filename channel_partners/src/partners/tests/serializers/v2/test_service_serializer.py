...
class TestServiceExtendedSerializer:
    ...
    def test_regular_service(self):
        serializer = ServiceExtendedSerializer(self.system.all_services.get(id=self.parent_local_recording.id))
        assert cast_uuid(serializer.data['id']) == self.parent_local_recording.id
        assert serializer.data['expirationDate'] is None
        assert serializer.data['hidden'] is False

    def test_demo_service(self):
        serializer = ServiceExtendedSerializer(self.system.all_services.get(id=self.parent_demo_analytic.id))
        assert cast_uuid(serializer.data['id']) == self.parent_demo_analytic.id
        assert parse(serializer.data['expirationDate']) == self.creation_date + relativedelta(months=10)
        assert serializer.data['hidden'] is False

    def test_demo_expired_service(self):
        serializer = ServiceExtendedSerializer(self.system.all_services.get(id=self.parent_demo_expired.id))
        assert cast_uuid(serializer.data['id']) == self.parent_demo_expired.id
        assert parse(serializer.data['expirationDate']) == self.creation_date + relativedelta(months=1)
        assert serializer.data['hidden'] is True

    def test_service_without_props(self):
        serializer = ServiceExtendedSerializer(self.system.all_services.get(id=self.service_without_props.id))
