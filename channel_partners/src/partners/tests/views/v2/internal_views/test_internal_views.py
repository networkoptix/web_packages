...
class TestCloudStorageUsageReport:
    ...
    def test_200_single_device_internal_service(self, mock_service_token_payload):
        data = {
            "usedDevices": {
                "cloudSystemId": str(self.system.system_id),
                "devices": [
        ...
        mocked_payload = mock_service_token_payload()
        response = self.client.post(path=self.path, data=data, format='json')
        assert response.status_code == 200
        assert ServiceUsage.objects.all().count() == 1
        assert ServiceUsage.objects.first().service_id == self.cloud_storage_service.id
    ...
    def test_401_single_device_internal_service(self, mock_service_token_payload, sa_jwt_payload_factory):
        data = {
            "usedDevices": {
                "cloudSystemId": str(self.system.system_id),
                "devices": [
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f'Service {uuid4()}')
        mocked_payload = mock_service_token_payload(sa_jwt_payload_factory(scope='[{"service": "cloud_db"}]'))
        response = self.client.post(path=self.path, data=data, format='json')
        assert response.status_code == 401
        mocked_payload.assert_called_once()


class TestAllOrgUsers:
    ...
    def setup(self,
        ...
        self.system = system_factory(organization=self.organization)
        self.group_user = sys_group_user_factory(organization=self.organization)
        self.path = reverse('v2:all_org_users')
        self.client = APIClient(SERVER_NAME=cloud_test_host.hostname)


    def test_403_not_internal_token(self, mock_internal_token_auth):
        token = mock_internal_token_auth()
    ...
    def test_401_not_internal_service(self, mock_service_token_payload, sa_jwt_payload_factory):
        mocked_service_payload = mock_service_token_payload(sa_jwt_payload_factory(scope='[{"service": "cloud_db"}]'))
        self.client.credentials(HTTP_AUTHORIZATION=f'Service {uuid4()}')
        response = self.client.get(self.path)
        assert response.status_code == 401
        mocked_service_payload.assert_called_once()

    def test_200_token_auth(self, mock_internal_token_auth):
        token = mock_internal_token_auth()
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {token.key}')
        response = self.client.get(self.path)
        assert response.status_code == 200
        assert self.cloud_user.email not in response.data['users']
        assert self.org_user.user.email in response.data['users']
        assert self.cp_user.user.email  in response.data['users']
        assert self.group_user.user.email in response.data['users']
        assert len(response.data['users']) == 3

    def test_200_service_auth(self, mock_service_token_payload):
        token = mock_service_token_payload()
        self.client.credentials(HTTP_AUTHORIZATION=f'Service {uuid4()}')
        response = self.client.get(self.path)
        assert response.status_code == 200