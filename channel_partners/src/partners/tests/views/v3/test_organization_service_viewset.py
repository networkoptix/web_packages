...
class TestOrganizationServiceViewSet:
    ...
    def setup_method(
        ...
        self.list_path = reverse('v3:organizations-owned-service-list',
                                 kwargs={'parent_lookup_organization': self.organization.id})
        self.detail_view_name = 'v3:organizations-owned-service-detail'
    ...
    def test_price_history(self, mock_auth_with_user):
        ...
        for i in range(10):
            ...
            service_props.save()
        path = reverse('v3:organizations-owned-service-price-history', kwargs=kwargs)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {uuid4()}')
        mock_auth_with_user(self.cp_user)
        response = self.client.get(path)
        assert response.status_code == 200
