...
class TestSystemGroupUserViewSet:
    ...
    def test_list_403(self, mock_auth_with_user, v3arf):
        with auto_execute_on_commit_callbacks():
    ...
    def test_list_200(self, mock_auth_with_user, v3arf):
        with auto_execute_on_commit_callbacks():
            view = SystemGroupUserViewSet.as_view(actions={'get': 'list'})
            request = v3arf.get('/')
    ...
    def test_retrieve_403(self, mock_auth_with_user, v3arf):
        with auto_execute_on_commit_callbacks():
    ...
    def test_retrieve_200(self, mock_auth_with_user, v3arf):
        with auto_execute_on_commit_callbacks():
            view = SystemGroupUserViewSet.as_view(actions={'get': 'retrieve'})
            request = v3arf.get('/')
    ...
    def test_create_403(self, mock_auth_with_user, v3arf, org_user_factory):
    ...
    def test_update_group_user_201(
            self, mock_auth_with_user, v3arf, sys_group_user_factory,
    ...
    def test_update_org_user_400(
            self, mock_auth_with_user, v3arf, org_user_factory,
            mock_account_status, mock_get_customization_request,
        ...
        assert 'cannot be added to group' in response.data['errors'][0]['detail']
        assert response.data['errors'][0]['attr'] == 'email'

    def test_create_201(self, mock_auth_with_user, v3arf, org_user_factory,
                        mock_account_status, mock_get_customization_request,
    ...
    def test_can_access(self, system_group_factory, sys_group_user_factory, v3arf, mock_auth_with_user):
        with auto_execute_on_commit_callbacks():
    ...
    def test_paginated_list(self, mock_auth_with_user, v3arf):
        with auto_execute_on_commit_callbacks():
...
class TestSystemGroupUserViewSetRetrieve:
    ...
    @pytest.fixture(autouse=True)
    def setup(self, channel_partner_factory, organization_factory, sys_group_user_factory,
              cloud_user_factory, org_user_factory, system_group_factory, v3arf, cp_user_factory):
        with auto_execute_on_commit_callbacks():
            self.cp = channel_partner_factory()
            ...
            self.sub_group_users = [sys_group_user_factory(organization=self.org, group=self.sub_group) for _ in
                                    range(5)]
            self.other_org = organization_factory(channel_partner=self.cp)
