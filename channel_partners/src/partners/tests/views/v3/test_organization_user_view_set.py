...
class TestOrganizationUserViewSet:
    ...
    def test_get_200(
            ...
            mock_auth_with_user, v3arf, random_email, mock_account_status,
    ...
    def test_create_200(self, organization_factory, org_user_factory, system_factory,
        ...
        request = v3arf.post('/', data=new_user_data, format='json')
    ...
    def test_update_200(self, organization_factory, org_user_factory, system_factory,
        ...
        request = v3arf.post('/', data=user_data, format='json')
        mock_account_status(email=random_email, active=False)
        mock_get_customization_request()
        notification_send_url = mock_post_notification()
        mock_auth_with_user(admin_user)
    ...
    def test_destroy_403(self, organization_factory, org_user_factory, system_factory,
        ...
        role = OrganizationRole.objects.get(name="Power User")
        user = org_user_factory(organization=org, role=role.name)
        request = v3arf.delete(reverse('v3:organizations-user-detail', kwargs={'parent_lookup_organization': org.id, 'email': user.user.email}))
    ...
    def test_user_validation(self, channel_partner_factory, cp_user_factory, organization_factory,
                             mock_auth_with_user, v3arf, org_user_factory):
        cp = channel_partner_factory()
        ...
        assert (f"User {cp_admin.user.email} has a role in the organization parent channel partner"
                in response.data['errors'][0]['detail'])
...
class TestOrganizationUserViewSetList:
    ...
    def setup_method(self, channel_partner_factory, organization_factory, system_group_factory,
                     mock_auth_with_user, v3arf, org_user_factory, cp_user_factory, sys_group_user_factory):
...
class TestOrganizationUserViewSetRetrieve:
    ...
    def test_2xx_get_group_user(self, mock_auth_with_user):
        ...
        assert response.data['groupRoles'][0]['roles'] == ['System Administrator']
        assert response.data['groupRoles'][0]['rolesIds'] == [str(OrganizationRoles.SYSTEM_ADMINISTRATOR)]
...
class TestOrganizationUserViewSetCreateUpdate:
    ...
    @pytest.fixture(autouse=True)
    def setup_method(self, channel_partner_factory, organization_factory, system_group_factory,
                     mock_auth_with_user, v3arf, org_user_factory, cp_user_factory,
    ...
    def test_create_new_user_200(self, mock_auth_with_user, v3arf):
        mock_auth_with_user(self.org_admin)
        request = v3arf.post('/', data=self.new_user_data, format='json')
    ...
    def test_create_cloud_user_200(self, mock_auth_with_user, v3arf):
        mock_auth_with_user(self.org_admin)
        request = v3arf.post('/', data=self.cloud_user_data, format='json')
        response = self.view(request, parent_lookup_organization=self.organization.id)
        assert response.status_code == 200
    ...
    def test_update_200(self, mock_auth_with_user, v3arf):
        mock_auth_with_user(self.org_admin)
        request = v3arf.post('/', data=self.org_viewer_data, format='json')
        response = self.view(request, parent_lookup_organization=self.organization.id)
        assert response.status_code == 200
    ...
    def test_create_200_cpal_admin(self, mock_auth_with_user, v3arf):
        mock_auth_with_user(self.cp_admin)
        request = v3arf.post('/', data=self.cloud_user_data, format='json')
        response = self.view(request, parent_lookup_organization=self.organization.id)
        assert response.status_code == 200
    ...
    def test_2xx_cpal_with_no_admins(self, mock_auth_with_user, v3arf):
        self.org_admin.delete()
        self.organization.channel_partner_access_level_id = None
        self.organization.save()
        mock_auth_with_user(self.cp_admin)
    ...
    def test_403_cpal_cp_reports_viewer(self, mock_auth_with_user, v3arf):
        mock_auth_with_user(self.cp_viewer)
        request = v3arf.post('/', data=self.new_user_data, format='json')
        response = self.view(request, parent_lookup_organization=self.organization.id)
        assert response.status_code == 403

    def test_403_no_cpal(self, mock_auth_with_user, v3arf):
        self.organization.channel_partner_access_level_id = None
        self.organization.save()
        mock_auth_with_user(self.cp_admin)
        request = v3arf.post('/', data=self.cloud_user_data, format='json')
    ...
    def test_403_parent_cp_user(self, mock_auth_with_user, v3arf):
        mock_auth_with_user(self.parent_cp_admin)
        request = v3arf.post('/', data=self.new_user_data, format='json')
        with transaction.atomic():
            response = self.view(request, parent_lookup_organization=self.organization.id)
    ...
    def test_update_403_self(self, mock_auth_with_user, v3arf):
        mock_auth_with_user(self.org_viewer)
        request = v3arf.post('/', data=self.org_viewer_data, format='json')
        with transaction.atomic():
            response = self.view(request, parent_lookup_organization=self.organization.id)
...
class TestOrganizationUserViewSetCreateUpdate:
    ...
    def test_2xx_non_admin_users(self, mock_auth_with_user, v3arf):
        mock_auth_with_user(self.org_admin)
        request = v3arf.post('/', data=self.users, format='json')
        response = self.view(request, parent_lookup_organization=self.organization.id)
        assert response.status_code == 200
        assert set(response.data['emails']) == set(self.users)
        assert self.organization.users.count() == 2 # admin and group admin

    def test_409_delete_with_admin_users(self, mock_auth_with_user, v3arf, org_user_factory):
        mock_auth_with_user(self.org_admin)
        request = v3arf.post('/', data=self.all_users, format='json')
        with transaction.atomic():
            response = self.view(request, parent_lookup_organization=self.organization.id)
        assert response.status_code == 409

    def test_2xx_with_admin_users(self, mock_auth_with_user, v3arf, org_user_factory):
        mock_auth_with_user(self.org_admin)
        org_user_factory(organization=self.organization)
        request = v3arf.post('/', data=self.all_users, format='json')
        response = self.view(request, parent_lookup_organization=self.organization.id)
    ...
    def test_2xx_with_non_existing(self, mock_auth_with_user, v3arf):
        mock_auth_with_user(self.org_admin)
        ...
        request = v3arf.post('/', data=[f'nonexist@networkoptix.com'], format='json')
        response = self.view(request, parent_lookup_organization=self.organization.id)
        assert response.status_code == 200
        assert response.data['emails'] == []

    def test_2xx_cpal_admin(self, mock_auth_with_user, v3arf):
        mock_auth_with_user(self.cp_admin)
        request = v3arf.post('/', data=self.users, format='json')
        response = self.view(request, parent_lookup_organization=self.organization.id)
        assert response.status_code == 200
    ...
    def test_403_self(self, mock_auth_with_user, v3arf):
        mock_auth_with_user(self.org_viewer)
        request = v3arf.post('/', data=[self.org_viewer.user.email], format='json')
        with transaction.atomic():
            response = self.view(request, parent_lookup_organization=self.organization.id)
        assert response.status_code == 403
...
class TestOrganizationUserViewSetFilter:
    ...
    def setup(self, channel_partner_factory, organization_factory, sys_group_user_factory,
        ...
        self.path = reverse('v3:organizations-user-list',
