...
class TestOrganizationServiceReportsViewSet:
    ...
    def test_403_forbidden(self, mock_auth_with_user, mocker):
        ...
        path = reverse('v3:organizations-reports-regular-detail-table', kwargs=path_kwargs)
    ...
    def test_expiring_detail_table(self, mock_auth_with_user, mocker):
        ...
        path = reverse('v3:organizations-reports-expiring-detail-table', kwargs=path_kwargs)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {uuid4()}')
        mock_auth_with_user(self.org_admin)
        response = self.client.get(path + self.period_query_param)
        assert response.status_code == 200
    ...
    def test_regular_service_report(self, mock_auth_with_user, mocker):
        ...
        path = reverse('v3:organizations-reports-regular-service-report', kwargs=path_kwargs)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {uuid4()}')
        mock_auth_with_user(self.org_admin)
        response = self.client.get(path + self.period_query_param)
        assert response.status_code == 200
    ...
    def test_expiring_service_report(self, mock_auth_with_user, mocker):
        ...
        path = reverse('v3:organizations-reports-expiring-service-report', kwargs=path_kwargs)
    ...
    #         "service_id": self.service.pk,
    #     }
    #     path = reverse('v3:organizations-reports-system-reports', kwargs=path_kwargs)
    #     self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {uuid4()}')
    #     mock_auth_with_user(self.org_admin)
    #     response = self.client.get(path + self.period_query_param)
    #     assert response.status_code == 200
    ...
    def test_system_regular_report(self, mock_auth_with_user, mocker):
        ...
        path = reverse('v3:organizations-reports-system-regular-detail-table', kwargs=path_kwargs)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {uuid4()}')
        mock_auth_with_user(self.org_admin)
        response = self.client.get(path + self.period_query_param)
        assert response.data
    ...
    def test_system_expiring_report(self, mock_auth_with_user, mocker):
        ...
        path = reverse('v3:organizations-reports-system-expiring-detail-table', kwargs=path_kwargs)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {uuid4()}')
        mock_auth_with_user(self.org_admin)
        response = self.client.get(path + self.period_query_param)
        assert response.data
    ...
    def test_usage_report(self, mock_auth_with_user, mocker):
        ...
        path = reverse('v3:organizations-reports-usage-report', kwargs=path_kwargs)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {uuid4()}')
        mock_auth_with_user(self.org_admin)
        response = self.client.get(path + self.period_query_param)
        assert response.status_code == 200
...
class TestChannelPartnerServiceReportsViewSet:
    ...
    def test_403_forbidden(self, mock_auth_with_user, mocker):
        ...
        path = reverse('v3:channelpartners-reports-usage-report', kwargs=path_kwargs)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {uuid4()}')
    ...
    def test_regular_detail_table(self, mock_auth_with_user, mocker):
        ...
        path = reverse('v3:channelpartners-reports-regular-detail-table', kwargs=path_kwargs)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {uuid4()}')
    ...
    def test_expiring_detail_table(self, mock_auth_with_user, mocker):
        ...
        path = reverse('v3:channelpartners-reports-expiring-detail-table', kwargs=path_kwargs)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {uuid4()}')
        mock_auth_with_user(self.channel_partner_admin)
        response = self.client.get(path + self.report_date_query_param)
        assert response.status_code == 200
    ...
    def test_regular_service_report(self, mock_auth_with_user, mocker):
        ...
        path = reverse('v3:channelpartners-reports-regular-service-report', kwargs=path_kwargs)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {uuid4()}')
        mock_auth_with_user(self.channel_partner_admin)
        response = self.client.get(path + self.report_date_query_param)
        assert response.status_code == 200
    ...
    def test_expiring_service_report(self, mock_auth_with_user, mocker):
        ...
        path = reverse('v3:channelpartners-reports-expiring-service-report', kwargs=path_kwargs)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {uuid4()}')
        mock_auth_with_user(self.channel_partner_admin)
        response = self.client.get(path + self.report_date_query_param)
        assert response.status_code == 200
    ...
    def test_channel_partner_usages_no_subcp(self, mock_auth_with_user, mocker):
        ...
        path = reverse('v3:channelpartners-reports-channel-partner-usages', kwargs=path_kwargs)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {uuid4()}')
        mock_auth_with_user(self.channel_partner_admin)
        response = self.client.get(path + self.report_date_query_param)
        assert response.status_code == 200
    ...
    def test_organization_usages_no_orgs(self, mock_auth_with_user, mocker, channel_partner_factory, ):
        ...
        path = reverse('v3:channelpartners-reports-organization-usages', kwargs=path_kwargs)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {uuid4()}')
        mock_auth_with_user(self.channel_partner_admin)
        response = self.client.get(path + self.report_date_query_param)
        assert response.status_code == 200
...
class TestOrganizationGenerateReport:
    ...
    def setup_method(self, organization_factory, org_user_factory, system_factory,
        ...
        self.view_name = 'v3:organizations-reports-generate-report'
