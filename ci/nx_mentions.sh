...
branding=$(grep -Ei "$BAN_LIST" -rl --exclude-dir=fonts --exclude={\*.{mock.ts,spec.ts,swf,png,gif},{commonPasswordsList,downloads}.json,test.ts,angular.json,environment.*} ../front_end/app) || true
if [[ -z ${branding} ]]
then
